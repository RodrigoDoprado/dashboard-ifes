...
const periodApi = axios.create({
  ...
  export const getPeriods = async (acronym: any): AxiosPromise<PeriodInterface[]> => {
  ...
  export const createPeriod = async (data: PeriodInterface): AxiosPromise<any> => {
        const res = await periodApi.post("/period",data);
        return res;
  };

  export const updatePeriod = async(data: PeriodInterface): AxiosPromise<any> => {
